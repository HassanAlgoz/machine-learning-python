...
- [**Regression Metrics**](./03_regression_metrics.ipynb) (where targets are **continuous**):
    - R-squared (default when you call `model.score()`) 
    - Mean Squared Error (MSE)
    - Mean Absolute Percentage Error (MAPE)
- [**Classification Metrics**](./04_classification_metrics.ipynb) (where targets are **categorical**):
...
To make use of the full dataset (especially in time-series predictions where recency it crucial); a final training on the entire dataset (including the test) is often done. But, no further testing is done after that. Instead, we report the number we got from the previous test.
