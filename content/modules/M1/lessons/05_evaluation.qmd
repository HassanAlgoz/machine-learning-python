...
**Loss or Error**: Is the internal objective function being minimized by the selected machine learning algorithm during training.
...
**Metrics or Scores**: are used to evaluate the model's performance from a practical or business perspective. They are not used to "train" the model; rather, to judge it's performance and compare it against other models.

Examples:

- **Regression** (where targets are **continuous**):
    - R-squared (default when you call `model.score()`) 
    - Mean Absolute Percentage Error (MAPE)
- **Classification** (where targets are **categorical**):
...
A separate discussion of **Regression Metrics** and another one for **Classification Metrics** will be covered later in the course.


## Verify model is learning relationship between X and Y in the data

The question is: How to verify the model is actually learning from the data given to it?
...
For instance, we could have been **lucky when we did our random split of our limited datase**t and isolated some of the easiest cases to predict in the testing set just by chance: the estimation of the testing error would be overly optimistic, in this case.
